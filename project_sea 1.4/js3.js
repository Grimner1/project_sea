...
//     общее кол-во НР;
    ...
    //      - добавил начальное сообщение в поле MessageChannel;
            ...
            // - Поменял модельку корабля;
            // - вернул значение для каждого корабля кол-во НР.
                    // Добавил сообщение которое выводится при уничтожении каждого корабля;
...
displayMessage('Адмирал! Пушки заряжены! Задайте координаты для выстрела в нижнем поле!');
                ...
                for (let i of allShipMass) {
                    for (let ii in i){
                        if (shoot === i[ii]) {
                            ...
                            i[ii] = 0;
                            i.shipHP -=1;
                            console.log(i.shipHP);
                            mass.push(shoot);
                            displayHit(shoot);
                            displayHitShip(ii);

                            if(i.shipHP === 0) {
                                displayMessage(`Есть попадание! АДМИРАЛ! ВЫ ПОТАПИЛ КОРАБЛЬ!`);
                            } else if (i.shipHP === 1) {
...
// ----------------------------------------Блок подсчёта и выдачи результатов
    ...
    if (isSunk === true) {
        if (shoots < 1){
            ...
            sniper = 'Командование будет не довольно...';
        } else {
